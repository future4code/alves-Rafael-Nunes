...
export class OrderBusiness {
    ...
    public createOrder = async (input: ICreateOrderInputDTO): Promise<ICreateOrderOutputDTO> => {
        ...
        if (pizzasInput.length === 0) {
            throw new ParamsError("Qauntidade insuficiente para realizar o pedido")
        }

        const pizzas = pizzasInput.map((pizza) => {
            if (pizza.quantity <= 0) {
                throw new ParamsError("O pedido deve ter no minimo 1 pizza")
            }
        ...
        for (let pizza of pizzas) {
            ...
            if (!price) {
                throw new NotFoundError("Sabor de  pizza não encontrada")
            }
